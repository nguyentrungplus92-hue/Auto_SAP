...
# ===== Helper: Lấy thông tin đăng nhập SAP =====

def get_sap_credentials(task, run_options=None):
    """
    Lấy thông tin đăng nhập SAP theo ưu tiên:
    - Chạy thủ công (run_options có username/password):
        → Dùng username đang login + password vừa nhập
        → Client vẫn lấy từ task.sap_user
    - Chạy auto (run_options=None):
        → Dùng task.sap_user như cũ
    Trả về None nếu không đủ thông tin.
    """
    if not task or not task.sap_user:
        return None

    if run_options and run_options.get('username') and run_options.get('password'):
        return {
            'client': task.sap_user.client,
            'username': run_options['username'],
            'password': run_options['password'],
        }

    return {
        'client': task.sap_user.client,
        'username': task.sap_user.username,
        'password': task.sap_user.password,
    }


def _try_delete_file(filepath, errors):
    """
    Xóa file chỉ khi không có lỗi dòng nào (100% thành công).
    Nếu có lỗi → giữ lại file để user kiểm tra và chạy lại.
    """
    if errors:
        log.info(f"  Giữ lại file vì có {len(errors)} dòng lỗi: {filepath}")
        return
    try:
        os.remove(filepath)
...
def default_handler(filepath, session=None, task=None, run_options=None):
...
def exchange_rate(filepath, session=None, task=None, run_options=None):
    """
    ...
    try:
        ...
        if task and task.param1:
            ...
            for item in items:
                ...
                if '/' in currency_pair:
                    ...
                    currency_to = 'VND'
                rows_data.append({
                    'kurst': 'M', 'gdatu': today, 'kursm': str(rate_value),
                    'fcurr': currency_to, 'tcurr': currency_from,
                })
        ...
        if not creds:
            return {"status": "success", "message": f"Validated (no SAP user): {rows_data}", "rows": len(rows_data)}

        log.info(f"  SAP User: {creds['client']}/{creds['username']}")
        time.sleep(2)

        with SapGuiClient(sap_entry_name='V1P', client_no=creds['client'], username=creds['username'], password=creds['password']) as sap:
            ...
            if res.get('ok'):
                ...
                return {"status": "success", "message": res.get('message', f"Đã nhập {len(rows_data)} tỷ giá vào OB08"), "rows": len(rows_data), "duration": round(duration, 2), "errors": errors}
            else:
                return {"status": "error", "message": res.get('message', 'Lỗi không xác định từ OB08'), "rows": 0, "duration": round(duration, 2)}
...
def md12_unfix_order(filepath, session=None, task=None, run_options=None):
    """
    ...
    try:
        ...
        creds = get_sap_credentials(task, run_options)
        if not creds:
            return {"status": "success", "message": f"Validated (no SAP user): {len(planned_orders)} orders", "rows": len(planned_orders)}
...
def mm02_update_vietnam_name(filepath, session=None, task=None, run_options=None):
    """
    ...
    try:
        ...
        if filepath.lower().endswith('.csv'):
            ...
            with open(filepath, 'r', encoding='utf-8-sig') as f:
                ...
                for row in reader:
                    if len(row) >= 2 and row[0].strip() and row[1].strip():
                        materials.append({'material': row[0].strip(), 'description': row[1].strip()})
        else:
            import openpyxl
            wb = openpyxl.load_workbook(filepath, data_only=True)
            ws = wb.active
            for row in ws.iter_rows(min_row=2):
                ...
                if material and description:
                    materials.append({'material': str(material).strip(), 'description': str(description).strip()})
            wb.close()
        ...
        creds = get_sap_credentials(task, run_options)
        if not creds:
            return {"status": "success", "message": f"Validated (no SAP user): {len(materials)} materials", "rows": len(materials)}
...
def mm02_update_dv_tinh(filepath, session=None, task=None, run_options=None):
    """
...
def me22_change_po_quantity(filepath, session=None, task=None, run_options=None):
    """
    ...
    try:
        ...
        if filepath.lower().endswith('.csv'):
            ...
            with open(filepath, 'r', encoding='utf-8-sig') as f:
                ...
                for row in reader:
                    if len(row) >= 3 and row[0].strip():
                        po_items.append({'po_number': row[0].strip(), 'item_number': row[1].strip(), 'quantity': row[2].strip()})
        else:
            import openpyxl
            wb = openpyxl.load_workbook(filepath, data_only=True)
            ws = wb.active
            for row in ws.iter_rows(min_row=2):
                ...
                if po_number and item_number and quantity:
                    po_items.append({'po_number': str(po_number).strip(), 'item_number': str(item_number).strip(), 'quantity': str(quantity).strip()})
            wb.close()
        ...
        creds = get_sap_credentials(task, run_options)
        if not creds:
            return {"status": "success", "message": f"Validated (no SAP user): {len(po_items)} items", "rows": len(po_items)}
...
def me12_update_marker(filepath, session=None, task=None, run_options=None):
    """
    ...
    try:
        ...
        if filepath.lower().endswith('.csv'):
            ...
            with open(filepath, 'r', encoding='utf-8-sig') as f:
                ...
                for row in reader:
                    if len(row) >= 2 and row[0].strip() and row[1].strip():
                        info_records.append({'vendor': row[0].strip(), 'material': row[1].strip(), 'marker': row[2].strip() if len(row) > 2 else ''})
        else:
            import openpyxl
            wb = openpyxl.load_workbook(filepath, data_only=True)
            ws = wb.active
            for row in ws.iter_rows(min_row=2):
                ...
                if vendor and material:
                    info_records.append({'vendor': str(vendor).strip(), 'material': str(material).strip(), 'marker': str(marker).strip() if marker else ''})
            wb.close()
        ...
        creds = get_sap_credentials(task, run_options)
        if not creds:
            return {"status": "success", "message": f"Validated (no SAP user): {len(info_records)} records", "rows": len(info_records)}
...
def me12_update_shipping_instruction(filepath, session=None, task=None, run_options=None):
    """
        ...
        if filepath.lower().endswith('.csv'):
            ...
            with open(filepath, 'r', encoding='utf-8-sig') as f:
                ...
                for row in reader:
                    if len(row) >= 4 and row[0].strip() and row[1].strip():
                        info_records.append({
                            'vendor': row[0].strip(), 'material': row[1].strip(),
                            'purch_org': row[2].strip(), 'plant': row[3].strip(),
            ...
            for row in ws.iter_rows(min_row=2):
                ...
                if vendor and material and purch_org and plant:
                    info_records.append({
                        'vendor': str(vendor).strip(), 'material': str(material).strip(),
                        'purch_org': str(purch_org).strip(), 'plant': str(plant).strip(),
...
def mk01_create_maker(filepath, session=None, task=None, run_options=None):
    """
    ...
    try:
        ...
        creds = get_sap_credentials(task, run_options)
        if not creds:
            return {"status": "success", "message": f"Validated (no SAP user): {len(vendors)} vendors", "rows": len(vendors)}
...
def cs02_upload_set_bom_pmg(filepath, session=None, task=None, run_options=None):
    """
    CS02 - Upload/Set BOM PMG

    File Excel/CSV:
    - Dòng 1: Tiêu đề (bỏ qua)
    - Cột A: MATNR, B: WERKS, C: STLAN, D: DATUV, E: IDNRK,
      F: MENGE, G: MEINS, H: POSTP, I: LGORT
    """
    ...
    try:
        ...
        creds = get_sap_credentials(task, run_options)
        if not creds:
            return {"status": "success", "message": f"Validated (no SAP user): {len(bom_items)} items", "rows": len(bom_items)}
...
def vl32n_del_inbound(filepath, session=None, task=None, run_options=None):
    """
    ...
    try:
        ...
        creds = get_sap_credentials(task, run_options)
        if not creds:
            return {"status": "success", "message": f"Validated (no SAP user): {len(deliveries)} deliveries", "rows": len(deliveries)}
...
def me52_del_pr(filepath, session=None, task=None, run_options=None):
    """
    ...
    try:
            ...
            with open(filepath, 'r', encoding='utf-8-sig') as f:
                ...
                for idx, row in enumerate(reader):
                    ...
                    if not banfn or not bnfpo:
                        skipped_rows.append(row_number)
                        continue
                    pr_items.append({'banfn': banfn, 'bnfpo': bnfpo})
        else:
            import openpyxl
            wb = openpyxl.load_workbook(filepath, data_only=True)
            ws = wb.active
        ...
        creds = get_sap_credentials(task, run_options)
        if not creds:
            return {"status": "success", "message": f"Validated (no SAP user): {len(pr_items)} PR items", "rows": len(pr_items)}
...
def mm02_update_ext_matl_group(filepath, session=None, task=None, run_options=None):
    """
...
def me22_change_price(filepath, session=None, task=None, run_options=None):
    """
    ME22 - Change PO Net Price

    File Excel/CSV:
    - Dòng 1: Tiêu đề (bỏ qua)
    - Cột A: PO_NUMBER - BẮT BUỘC
    - Cột B: ITEM_NUMBER - BẮT BUỘC
    - Cột C: PRICE - BẮT BUỘC
    """
    ...
    try:
            ...
            with open(filepath, 'r', encoding='utf-8-sig') as f:
                ...
                for idx, row in enumerate(reader):
                    ...
                    if not po_number or not item_number or not price:
                        skipped_rows.append(row_number)
                        continue
                    po_items.append({'po_number': po_number, 'item_number': item_number, 'price': price})
        else:
            import openpyxl
            wb = openpyxl.load_workbook(filepath, data_only=True)
            ws = wb.active
        ...
        creds = get_sap_credentials(task, run_options)
        if not creds:
            return {"status": "success", "message": f"Validated (no SAP user): {len(po_items)} PO items", "rows": len(po_items)}
...
def qa11_ud_code(filepath, session=None, task=None, run_options=None):
    """
    QA11 - Record Usage Decision

    File Excel/CSV:
    - Dòng 1: Tiêu đề (bỏ qua)
    - Cột A: PRUEFLOS - BẮT BUỘC
    - Cột B: VCODE - BẮT BUỘC
    - Cột C: VCODEGRP - BẮT BUỘC
    """
    ...
    try:
            ...
            with open(filepath, 'r', encoding='utf-8-sig') as f:
                ...
                for idx, row in enumerate(reader):
                    ...
                    if not prueflos or not vcode or not vcodegrp:
                        skipped_rows.append(row_number)
                        continue
                    items.append({'prueflos': prueflos, 'vcode': vcode, 'vcodegrp': vcodegrp})
        else:
            import openpyxl
            wb = openpyxl.load_workbook(filepath, data_only=True)
            ws = wb.active
        ...
        creds = get_sap_credentials(task, run_options)
        if not creds:
            return {"status": "success", "message": f"Validated (no SAP user): {len(items)} inspection lots", "rows": len(items)}
...
def vl32n_change_delivery_date(filepath, session=None, task=None, run_options=None):
    """
    ...
    try:
            ...
            with open(filepath, 'r', encoding='utf-8-sig') as f:
                ...
                for idx, row in enumerate(reader):
                    ...
                    if not vbeln or not lfdat_la:
                        skipped_rows.append(row_number)
                        continue
                    items.append({'vbeln': vbeln, 'lfdat_la': lfdat_la})
        else:
            import openpyxl
            wb = openpyxl.load_workbook(filepath, data_only=True)
            ws = wb.active
        ...
        creds = get_sap_credentials(task, run_options)
        if not creds:
            return {"status": "success", "message": f"Validated (no SAP user): {len(items)} deliveries", "rows": len(items)}
...
def mm02_update_maximum_lot_size(filepath, session=None, task=None, run_options=None):
    """
    MM02 - Update Maximum Lot Size (MRP 1)

    File Excel/CSV:
    - Dòng 1: Tiêu đề (bỏ qua)
    - Cột A: MATNR - BẮT BUỘC
    - Cột B: WERKS - BẮT BUỘC
    - Cột C: BSTMA - BẮT BUỘC
    """
            ...
            with open(filepath, 'r', encoding='utf-8-sig') as f:
                ...
                for idx, row in enumerate(reader):
                    ...
                    if not matnr or not werks or not bstma:
                        skipped_rows.append(row_number)
                        continue
                    materials.append({'matnr': matnr, 'werks': werks, 'bstma': bstma})
        else:
            import openpyxl
            wb = openpyxl.load_workbook(filepath, data_only=True)
            ws = wb.active
