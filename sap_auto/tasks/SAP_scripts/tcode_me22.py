...
class TCodeME22:
    ...
    def run_price(self, po_items: list) -> dict:
        ...
        Args:
            po_items: List dict với keys:
                - po_number: PO Number (BẮT BUỘC)
                - item_number: Item Number (BẮT BUỘC)
                - price: Net Price (BẮT BUỘC)
        ...
        return {
            'ok': True,
            'status': 'success',
            'message': f"Hoàn tất. Đã cập nhật giá cho {processed}/{total} PO items",
            'processed': processed,
            'total': total,
            'error_at_row': None,
            'error_detail': None
